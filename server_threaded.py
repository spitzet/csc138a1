...
#
# This is a multi-threaded server implementation
#
...
class HttpServer ():
    ...
    def stop(self):
        ...
        self._logger.info('Waiting for client threads to stop')
    ...
    def _wait(self):
        ...
        while (True):
            ...
            self._logger.info('Starting new client thread')
            client_thread = HttpServerClientThread(client, self)
            ...
            for thread in self._clientThreads:
                if not thread.isAlive():
                    self._clientThreads.remove(thread)
                    thread.join()
...
class HttpServerClientThread (threading.Thread):
    ...
    def run(self):
        ...
        server._logger.debug('Method: ' + method)
        server._logger.debug('Request URI: ' + uri)
        server._logger.debug('Request Body: ' + body)

        # Check to see if the request method is implemented and if so, pass off handling to the appropriate handler function
        if (method in server._methodHandlers):
            server._logger.info('Handling ' + method + ' request for URI: ' + uri)
            (response_code, response) = server._methodHandlers[method](uri, body)
        else:
            # If we don't have a support method, generate the appropriate HTTP response status code
            response_code = 501;
            response = server._genErrorHtml(response_code, 'Method ' + method + ' is not implemented on this server')
