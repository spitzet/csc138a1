...
class HttpServer ():
	...
	def start(self):
		...
		try:
			self._logger.info('Starting HTTP server on ' + host + ':' + port)
	...
	def _genErrorHtml(self, response_code, message):
		...
		return '<!DOCTYPE HTML PUBLIC "-//IETF//DTD HTML 2.0//EN">\n<html><head>\n<title>{code} {error}</title>\n</head><body>\n<h1>{error}</h1>\n<p>{message}</p>\n</body></html>'.format(code=response_code, error=self._responseCodes[response_code], message=message)
	...
	def _parseURI(self, uri):
		...
		root = os.path.abspath(self.config['wwwroot'])

		original_filename = uri_parts[0];
		basename = os.path.basename(original_filename)
		
		relative_filename = os.path.relpath(root + '/' + original_filename, root)
		absolute_filename = os.path.abspath(root + '/' + relative_filename)

		if (os.path.commonprefix([absolute_filename, root]) == root):
			filename = os.path.abspath(absolute_filename)

		# Force the use of an index file (e.g. index.html) when the requested URI is a directory
		if (os.path.isdir(filename) and self.config['indexfile'] != False):
